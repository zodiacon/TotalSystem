...
#include <ntddk.h>
#include <wdmsec.h>
#include "KTotalSysPublic.h"

#define DRIVER_PREFIX "KTotalSys"
...
extern "C" NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING) {
	...
	UNICODE_STRING devName = RTL_CONSTANT_STRING(L"\\Device\\KTotalSys");
	...
	UNICODE_STRING symName = RTL_CONSTANT_STRING(L"\\??\\KTotalSys");
...
NTSTATUS ObjExpDeviceControl(PDEVICE_OBJECT, PIRP Irp) {
	...
	switch (dic.IoControlCode) {
		case IOCTL_KTOTALSYS_OPEN_OBJECT:
		{
		...
		case IOCTL_KTOTALSYS_DUP_HANDLE:
		{
			if (Irp->AssociatedIrp.SystemBuffer == nullptr) {
				status = STATUS_INVALID_PARAMETER;
				break;
		...
		//case IOCTL_KTOTALSYS_OPEN_WINSTA_BY_NAME:
		//	ObjectType = *ExWindowStationObjectType;
		case IOCTL_KTOTALSYS_OPEN_EVENT_BY_NAME:
			ObjectType = *ExEventObjectType;
		case IOCTL_KTOTALSYS_OPEN_DESKTOP_BY_NAME:
			ObjectType = *ExDesktopObjectType;
		case IOCTL_KTOTALSYS_OPEN_SEMAPHORE_BY_NAME:
			if (ObjectType == nullptr)
				ObjectType = *ExSemaphoreObjectType;
		case IOCTL_KTOTALSYS_OPEN_JOB_BY_NAME:
		{
			if (ObjectType == nullptr)
		...
		case IOCTL_KTOTALSYS_OPEN_PROCESS:
		case IOCTL_KTOTALSYS_OPEN_THREAD:
		case IOCTL_KTOTALSYS_OPEN_PROCESS_TOKEN:
		{
			if (Irp->AssociatedIrp.SystemBuffer == nullptr) {
				status = STATUS_INVALID_PARAMETER;
				break;
			...
			if (dic.IoControlCode == IOCTL_KTOTALSYS_OPEN_PROCESS || dic.IoControlCode == IOCTL_KTOTALSYS_OPEN_PROCESS_TOKEN) {
				id.UniqueProcess = UlongToHandle(data->Id);
				const ACCESS_MASK PROCESS_QUERY_INFORMATION = 0x400;
				status = ZwOpenProcess(&hObject,
					dic.IoControlCode == IOCTL_KTOTALSYS_OPEN_PROCESS_TOKEN ? PROCESS_QUERY_INFORMATION : data->AccessMask, &attr, &id);
			}
			else {
				id.UniqueThread = UlongToHandle(data->Id);
				status = ZwOpenThread(&hObject, data->AccessMask, &attr, &id);
			}
			if (dic.IoControlCode == IOCTL_KTOTALSYS_OPEN_PROCESS_TOKEN) {
		...
		case IOCTL_KTOTALSYS_GET_VERSION:
			if (Irp->AssociatedIrp.SystemBuffer == nullptr) {
				status = STATUS_INVALID_PARAMETER;
				break;
			}
		...
		case IOCTL_KTOTALSYS_GET_OBJECT_ADDRESS:
			if (Irp->AssociatedIrp.SystemBuffer == nullptr) {
				status = STATUS_INVALID_PARAMETER;
				break;
			}
